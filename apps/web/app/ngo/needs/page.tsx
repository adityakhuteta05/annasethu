    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 border-slate-200 pb-4">
        ...
        <Link
          ...
          className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 shadow-xs transition-colors self-start sm:self-auto"
        >
      ...
      {/* Filter Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-2xl bg-white border border-slate-200 border-slate-200 shadow-xs">
        ...
        <button
          onClick={fetchNeeds}
          className="p-1.5 rounded-lg border border-slate-200 text-slate-500 hover:text-[#2d6a4f]"
          title="Refresh"
        >
      ...
      <div className="bg-white rounded-3xl border border-slate-200 border-slate-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 bg-white border-b border-slate-200 border-slate-200 text-slate-500 uppercase text-[10px] tracking-wider font-bold">
              <tr>
            ...
            <tbody className="divide-y divide-[#e5dec9]/60 dark:divide-[#2d3239]/60">
              {filteredNeeds.map((need) => (
                <tr key={need.id} className="hover:bg-white dark:hover:bg-[#14171a]/50 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-slate-900 dark:text-[#4f9d3a]">
                    {need.id}
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <div className="font-semibold text-slate-900 dark:text-white">{need.food_category}</div>
                    <span className="text-[10px] text-slate-500">{need.dietary_type}</span>
                  </td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      <Link
                        ...
                        className="px-2.5 py-1 rounded-lg bg-[#2d6a4f] text-white text-[11px] font-bold hover:bg-emerald-700 shadow-xs"
                      >
                        [ VIEW MATCHES ]
                      </Link>
                      ...
                      {need.status === 'ACTIVE' && (
                        <button
                          ...
                          className="px-2 py-1 rounded-lg border border-slate-200 text-[11px] font-semibold text-slate-500 hover:bg-gray-50"
